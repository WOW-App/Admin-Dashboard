...
async function getUser(){
   ...
   console.log("data is",data)
    var config = {
                method: 'POST',
                url: 'http://localhost:6969/dash/user',
                headers: { 
...
export default function DeleteUser() {
    ...
    return (
        ...
        <div className="del-form">
            ...
            {user==true && <div className="user-detail"><h1>user details are as below</h1> </div>}
            {user =="notFound" && <h1> User Not Found</h1>}
            <button className="act-btn" onClick={()=>{getUser();if(user){setUser(true)}else{setUser("notFound")}}}>Delete User</button>

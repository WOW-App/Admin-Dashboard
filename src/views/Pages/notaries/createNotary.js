...
export default function CreateNotary(){
    const [addNotary,setAddNotary]=React.useState(null)
    return(
        <><form>
         <div className="form-check">
  
  <label for="exampleInputPassword1">is_athome</label>
  <input type="checkbox" onChange={(e)=>{if(e.target.checked){dataObj.is_athome=true}else{dataObj.is_enterprise_product=false}}} className="form-check-input" id="exampleInputPassword1" placeholder=""/>
</div>
        <div className="form-check">
  
  <label for="exampleInputPassword1">is_available</label>
  <input type="checkbox" onChange={(e)=>{if(e.target.checked){dataObj.is_available=true}else{dataObj.is_enterprise_product=false}}} className="form-check-input" id="exampleInputPassword1" placeholder=""/>
</div>
...
<div className="form-check">
    <input type="checkbox" onChange={(e)=>{if(e.target.checked){setAddNotary(true)}else{setAddNotary(null)}}} className="form-check-input" id="exampleCheck1"/>
    <label className="form-check-label" for="exampleCheck1" >
        The above Notary info is valid
    </label>
  </div>
<button type="submit" className="btn btn-primary" disabled={!addNotary} onClick={()=>{notaryCreation()}}>Add Notary</button>
